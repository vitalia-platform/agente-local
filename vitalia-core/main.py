...
def build_orchestrator():
    ...
    logger.log_event("system_log", "orchestrator", {"action": "routing", "architect_url": NO1_URL, "engineer_url": NO2_URL})
    print(f"🔗 Rotas Ativas - Architect: {NO1_URL} | Engineer: {NO2_URL}")
    ...
    architect = AssistantAgent(
        ...
        system_message=(
            ...
            "Responda com __VITALIA_ABORT__ quando o objetivo da sprint estiver concluído ou se houver um erro técnico irrecuperável. "
    ...
    engineer = AssistantAgent(
        ...
        system_message=(
            ...
            "com `update_sprint_state`. Responda com __VITALIA_ABORT__ quando a tarefa estiver concluída ou se houver um erro técnico irrecuperável. "
            "[CRITICAL] NUNCA escreva blocos de código JSON simulando a chamada de ferramentas. "
            "Sempre utilize o mecanismo NATIVO de Function Calling da API para acionar ferramentas."
        ),
    )
    ...
    # ── Critério de parada ────────────────────────────────────────────────
    termination = (
        MaxMessageTermination(max_messages=10) | TextMentionTermination("__VITALIA_ABORT__")
    )
