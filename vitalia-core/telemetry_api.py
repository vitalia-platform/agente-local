            ...
            if events:
                for stream, messages in events:
                    for msg_id, msg_data in messages:
                        last_id = msg_id
                        if logger and "payload" in msg_data:
                            dec = logger.decrypt_payload(msg_data["payload"])
                            msg_data["payload"] = json.dumps(dec)
...
# --------- NODE INVENTORY & QUEUES ---------

@app.get("/api/nodes")
async def get_nodes(username: str = Depends(get_current_user)):
    r = redis_async.Redis.from_url(REDIS_URL, decode_responses=True)
    try:
        keys = await r.keys("vitalia:nodes:*")
        nodes = []
        for key in keys:
...
@app.get("/api/queues")
async def get_queues(username: str = Depends(get_current_user)):
    r = redis_async.Redis.from_url(REDIS_URL, decode_responses=True)
    try:
        # A list of streams might be obtained by keys, but we need to check type
        keys = await r.keys("vitalia:*")
        queues = []
        for key in keys:
...
async def get_queue_messages(queue_name: str, count: int = 50, username: str = Depends(get_current_user)):
    r = redis_async.Redis.from_url(REDIS_URL, decode_responses=True)
    try:
        # Prevent accessing non-vitalia keys for security
        if not queue_name.startswith("vitalia"):
            raise HTTPException(status_code=403, detail="Acesso negado a esta fila")
            
        messages = await r.xrevrange(queue_name, max='+', min='-', count=count)
        result = []
        for msg_id, payload in messages:
            if logger and "payload" in payload:
                dec = logger.decrypt_payload(payload["payload"])
                payload["payload"] = json.dumps(dec)
            result.append({
                "id": msg_id,
                "payload": payload
            })
        return {"queue": queue_name, "messages": result}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        await r.aclose()
