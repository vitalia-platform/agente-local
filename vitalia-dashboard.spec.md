...
- [ ] O sistema deve implementar uma arquitetura **Event-Driven Unificada**: Os agentes (Nó 1) e o orquestrador enviarão todas as métricas, logs e conversas para um **único stream/fila no Redis** (ex: `vitalia_events`).
- [ ] Cada evento será tipado rigidamente (`type: conversation | telemetry | system_log | reasoning | tool_call`), permitindo que o Dashboard filtre o que exibir na tela separadamente.
- [ ] O `telemetry_api.py` (atuando como Backend Bridge) deve assinar esse stream unificado e repassar os dados ao Frontend através de **WebSockets**.
...
- [ ] **Segurança (Autenticação Obrigatória):** O Dashboard rodará na máquina host, e o acesso será protegido por uma **Senha Mestra** simples que gera um token JWT. Nenhum comando de controle (Kill/Restart) será aceito sem esse token.
- [ ] **Persistência Auditável (Sharding):** Dados auditáveis (logs, reasoning, conversas) oriundos do Event Bus devem ser persistidos fisicamente utilizando a técnica de *shards* (mesmo padrão do contexto de sessão). 
    - Destino: `.specify/memory/data_storage/shards/`
    - Formato: Arquivos fragmentados por máquina, ex: `<machine_id>_audit.jsonl`, preparando o terreno para consolidação futura em um repositório git separado.
- [ ] **Topologia Distribuída:** Cada nó (Nó 1 e Nó 2) pode injetar dados no Barramento Único, mas o Control Plane centraliza as leituras, acessando diretamente o Docker local (host) para reiniciar serviços.
